...
void clampedExpVector(float *values, int *exponents, float *output, int N) {
  ...
  __cs149_vec_float top = _cs149_vset_float(9.999999);

  for (int i = 0; i < N; i += VECTOR_WIDTH) {
    int step = (i + VECTOR_WIDTH) < N ? VECTOR_WIDTH : N - i;
    __cs149_mask mask = _cs149_init_ones(step);

    __cs149_vec_float val_vec;
    ...
    __cs149_mask vgt_res = _cs149_init_ones();
    _cs149_vgt_int(vgt_res, exp_vec, zeros, mask);

    while (_cs149_cntbits(vgt_res) > 0) {
      _cs149_vmult_float(result, result, val_vec, vgt_res);

      _cs149_vsub_int(exp_vec, exp_vec, one, vgt_res);
      _cs149_vgt_int(vgt_res, exp_vec, zeros, vgt_res);
    }

    __cs149_mask is_top = _cs149_init_ones();
    _cs149_vgt_float(is_top, result, top, mask);

    _cs149_vset_float(result, 9.999999, is_top);
